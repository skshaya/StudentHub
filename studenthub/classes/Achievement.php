...
class Achievement extends Post{
    ...
    public function loadAchievements($id){
        $query= "SELECT DISTINCT p.*,fname,lname,pro_pic,(SELECT liked FROM user_likes_post ulp WHERE ulp.user_id=$id AND p.post_id=ulp.post_id) AS liked "
                    . "FROM (SELECT x.*,achieve_time,image FROM post x,achievement y,post_has_image z WHERE x.post_id=y.post_id AND x.post_id=z.post_id) p,user u "
    ...
    public function addAchievement($caption,$postType,$userId,$aDate,$image){
     
        $query="CALL new_achieve('$caption','$postType',$userId,'$aDate','$image');";
    ...
    public function viewAchievement($id,$post_id){
